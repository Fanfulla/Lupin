...
<b>Claude Code, on any model.</b><br>
<sub>The gentleman router: it borrows the Claude Code harness and lends it to other models.</sub>

<br><br>

[![CI](https://github.com/Fanfulla/Lupin/actions/workflows/ci.yml/badge.svg)](https://github.com/Fanfulla/Lupin/actions/workflows/ci.yml)
...
**`switch Lupin profile: <name>`**. Lupin publishes one per profile, and picking
one moves the active profile from inside the session, with the conversation
intact. That is the answer to running out of quota mid-task, and it is exactly
how this was verified: Kimi answered `usage limit for this billing cycle`, the
picker moved the session to ChatGPT, and the next request was served.

<details>
<summary>One honest limit of the picker</summary>

The client re-sends the picked id on every later turn, so Lupin acts on it only
when it **changes**. Consequence: pick `switch: B` from the picker, then switch
to A from the CLI or the TUI, and the picker cannot bring you back to B, because
re-picking the row it still shows as selected sends the same id and no gesture is
seen. Any other surface works (`lupin use B`, the TUI, or picking a different row
first). The alternative was letting every turn of an old session drag the active
profile back, which would mean no other surface could ever hold a switch.
...
<div align="center">
<img src="https://raw.githubusercontent.com/Fanfulla/Lupin/main/docs/doctor.gif" alt="lupin doctor scoring the ChatGPT subscription 10/10" width="900">
<br>
<sub>A real run against the ChatGPT subscription. Only the waiting is edited: the session took 116 seconds and is held for two.</sub>
</div>
...
<details>
<summary><b>What the doctor has actually measured</b> (dates included, because a score without one is a rumour)</summary>
...
<details>
<summary><b>The four rules that engine follows</b></summary>
...
The sidecar is not on npm and never will be: it stays out of the JavaScript
runtime by design. Build it from a clone, once, then launch it with the bare
command:
...
```
⣀⣤⣶⣾⣿⣷⣄⠀⠀⠀⠀⠀⠀⠀⠀⠀  L U P I N  v0.1.2   the gentleman router
...
<details>
<summary><b>Statusline install and every segment it draws</b></summary>

Opt-in, always: **Lupin never writes your `settings.json`** (ADR-11). Copy [`examples/statusline.ps1`](examples/statusline.ps1) (Windows) or [`examples/statusline.sh`](examples/statusline.sh) (macOS and Linux, needs `jq`) into `~/.claude/` and register it:
...
<details>
<summary><b>What does it cost me to run?</b></summary>
...
<details>
<summary><b>Why is Anthropic not in the provider list?</b></summary>
...
| File | Content |
|---|---|
| [docs/NEXT-STEPS.md](docs/NEXT-STEPS.md) | **Start here**: current state, how to verify it, what to do next |
| [DESIGN.md](DESIGN.md) | Vision, prior art, positioning, risks |
| [docs/DECISIONS.md](docs/DECISIONS.md) | ADR log: every decision, the why, the rejected alternatives |
| [docs/SPEC-TRANSLATION.md](docs/SPEC-TRANSLATION.md) | Translation core: mapping, SSE, errors, acceptance fixtures |
| [docs/SPEC-PROVIDERS.md](docs/SPEC-PROVIDERS.md) | Provider registry, profiles, slot mapping, quirks |
| [docs/SPEC-CLI.md](docs/SPEC-CLI.md) | CLI, `doctor`, security, UX |
| [docs/ROADMAP.md](docs/ROADMAP.md) | Milestones, verification criteria, next steps |
...
| [docs/DESIGN-TRANSLATORS-DEDICATED.md](docs/DESIGN-TRANSLATORS-DEDICATED.md) | The two subscription translators |
...
<div align="center">
<br>
<sub>The portrait is Arsene Lupin as Leo Fontan drew him in 1908 for <i>Arsene Lupin contre Herlock Sholmes</i>. Public domain, like the books.</sub>
<br><br>
