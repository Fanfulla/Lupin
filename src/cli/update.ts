...
export type UpdatePlan =
  | { kind: 'upToDate' }
  /** One of the two versions does not parse: decide nothing, say both. */
  | { kind: 'incomparable' }
...
export async function updateCommand(fetchImpl: typeof fetch = fetch): Promise<number> {
  ...
  const plan = planUpdate({ current: CLIENT_VERSION, latest, ...(sidecarPath !== undefined ? { sidecarPath } : {}), cargoAvailable });

  if (plan.kind === 'upToDate') {
    console.log(`already the latest: ${CLIENT_VERSION} (registry says ${latest})`);
    return 0;
  }
