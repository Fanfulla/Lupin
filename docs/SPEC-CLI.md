...
### `lupin init` (REMOVED 2026-08-12, ADR-51)
The interactive wizard is gone: setup lives in the TUI hub (bare `lupin`), which drives the same verification and persistence functions through the authenticated control API. Everything the wizard did is a hub capability now: provider pick with honest descriptions, masked key input, the 1-token connectivity test, the save-anyway choice after a failed check, local-runtime discovery with model picks and warnings, the economy preset, and the §4septies offers. Idempotency is unchanged: adding a provider adds a profile, it never destroys one.

### Bare `lupin`: TUI-native onboarding

With a TTY and an available `lupin-tui` sidecar, bare `lupin` opens the hub. With no config and no usable sidecar (or no TTY), bare `lupin` still opens a first-run path (2026-08-12): it starts the same bootstrap daemon the TUI would get and prints the authenticated control calls, ready to paste, so §Headless setup works from zero; nothing is persisted until a provider verifies. With a config, the hub first ensures the daemon is running (the same `ensureDaemon` as `lupin run`, 2026-08-12): a dashboard that opens on "daemon DOWN" would let every gesture fail at its last step. When no config exists, the hub starts an empty in-memory daemon and passes its loopback port and generated local token to the TUI. No config or credentials are persisted until a provider has been successfully verified; normal daemon lifecycle files such as the pidfile, log and watchdog pidfile may be written at startup. The first saved profile must keep that bootstrap port and local token so the running TUI and daemon stay authenticated after the config appears.
...
Since ADR-51 (2026-08-12) this is the ONLY setup surface. Beyond the hosted flows above it covers: **local-runtime discovery and setup** (`POST /v1/lupin/discover-local` probes the live server and returns each chat model with its window, provenance, tools/vision flags and the too-small verdict; `POST /v1/lupin/setup-local` writes the profile with the main/light picks, the persistable windows, and the vision/longContext routes ONLY on explicit opt-in, §4septies); **the economy preset** where the defaults declare one (`economy` on `setup-key`, the catalogue row carries the description); **save-anyway** after a failed key check (`saveAnyway` on `setup-key`: the error is shown first, nothing is stored without the explicit choice); **official-CLI credential import** (the catalogue row says `importAvailable`, the login body takes `importIfAvailable`, and the job then verifies and persists without a browser); **one account label per OAuth login** (`account`, `[A-Za-z0-9._-]{1,32}`, refused otherwise, §4nonies) on login and logout; and **the failover offer**, asked AFTER the setup succeeded, because the answer should follow the verdict (a failover named for a key the provider then rejects is a wasted question): the TUI writes it with `POST /v1/lupin/failover` (`{ profile, failover? }`, set or clear, validated against the existing profiles, self-reference refused, never auto-activated). The `failover` field on both setup routes remains for one-call headless setups. The TUI never spawns CLI child commands; every front end reuses the same verification and persistence functions behind the authenticated control API. Dropped with the CLI verbs and recorded honestly: the post-init recommended-skills offer (the docs and the tutorial cover the same ground).

The hub also aims slots (2026-08-12): `POST /v1/lupin/slots` takes `{ profile, opus?, sonnet?, haiku?, contextWindows? }` with at least one slot named, writes the model strings **as given and never checked** (the `use --opus` rule: nothing local can know which ids a plan accepts, and an invented validation would be worse than none; the surface says so), refuses an unknown profile, and goes through the same load-mutate-save path as every other write, so the daemon hot-reloads it. The optional `contextWindows` map (2026-08-13, ADR-52) merges served-truth windows into the profile in the same atomic write, which is how a catalogue pick arms §4quinquies without a second call. In the TUI it is the `m` key on the selected profile row; catalogue rows that already have a profile with that id are labelled `configured`, so adding over an existing profile is a visible choice rather than a surprise.

Since 2026-08-13 (ADR-52) the hub is also catalogue-assisted: `POST /v1/lupin/discover-catalog { providerId }` returns the normalized public model list of a provider whose registry entry declares `catalogApi` (SPEC-PROVIDERS §3quinquies; OpenRouter first). The TUI uses it in three places: the quick-model gesture (`t`: one id, typed, searched or pasted, aimed at all three slots with per-slot exclusion), the slot editor (`m`: live suggestions under the focused field, Tab completes), and the agents model target. The catalogue informs and never gates; with no catalogue every input degrades to the plain text field. `POST /v1/lupin/agents/wire { name, unset?, cwd? }` exposes the ADR-48 wire so the TUI can offer it after saving a named route (SPEC-PROVIDERS §4decies).
...
### `lupin login <provider>` / `lupin logout <provider>` (REMOVED 2026-08-12, ADR-51)
OAuth login and logout live in the TUI hub, through the control API's job-based login (`POST /v1/lupin/login`, polled at `GET /v1/lupin/login/:id`) and `POST /v1/lupin/logout`. Every guarantee of the CLI flow carries over unchanged: the token is verified with a real request BEFORE being saved, a provider with a suspension warning blocks on explicit risk acceptance, the official-CLI import is offered when available, the account label (§4nonies) picks the store key and the derived profile name, and a login whose profile cannot be created rolls the stored token back. Tokens live only in the credential store (600 file or OS keychain), never in the config.
...
2. **The package**: before `npm i -g lupin-code@latest`, the updater takes the same atomic lifecycle lock used by every daemon startup path, records whether the Lupin daemon is running, and verifies that every live PID in the lifecycle files has both the exact expected Lupin entrypoint and the random ownership token written when that process was spawned. It checks each identity immediately before stopping the recorded watchdog first and the recorded daemon second, then waits for both processes to exit. A legacy, missing or mismatched process identity aborts without stopping that process. A stale lock is reported for manual inspection and is never deleted automatically. The lock stays held through npm and recovery, so a concurrent `lupin run` cannot recreate either process inside the replacement window. This is required on Windows, where either process can keep the global package directory locked through its working directory or loaded native keyring module and make npm fail with `EBUSY` while renaming `node_modules/lupin-code`. npm is then spawned with inherited stdio so its own output (progress, permission errors) reaches the user unfiltered. If the daemon was running before the update, it is started again after a successful install. Detached daemon and watchdog processes use the Lupin state directory, not the package directory, as their working directory.
...
Every step reports its outcome in words; a failed step never claims the ones after it. If npm fails after a running daemon was stopped, the updater attempts to start the installed or restored daemon again and reports that recovery separately. The decision of what to do is a pure, tested function of the observed state (versions, sidecar presence, toolchain presence); the executor only carries it out.
