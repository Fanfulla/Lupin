...
The hub also aims slots (2026-08-12): `POST /v1/lupin/slots` takes `{ profile, opus?, sonnet?, haiku? }` with at least one slot named, writes the model strings **as given and never checked** (the `use --opus` rule: nothing local can know which ids a plan accepts, and an invented validation would be worse than none; the surface says so), refuses an unknown profile, and goes through the same load-mutate-save path as every other write, so the daemon hot-reloads it. In the TUI it is the `m` key on the selected profile row; catalogue rows that already have a profile with that id are labelled `configured`, so adding over an existing profile is a visible choice rather than a surprise.
