...
**The phantom-logout split-brain is fixed (2026-08-06, ADR-43).** The credential store used to promote a secret from the file into the OS keychain on first read and delete the file copy, so a file-only process (this repository without `@napi-rs/keyring`) reported `no OAuth credentials` about a credential that was intact in the keychain. Promotion (and any keychain-mode overwrite of a stale file copy) now buries the file copy under a non-secret marker, and the miss-path errors read it: the message names the OS keychain, the move date, and the real fix (`npm i @napi-rs/keyring` here, or a re-login). If you still see a plain `no OAuth credentials` in a checkout, the credential really is absent.
