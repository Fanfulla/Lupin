...
- **Lazy promotion**: with the keychain active and a key found only in the file, the read writes it into the keychain, **verifies it by reading it back**, and only after that verification replaces the file copy with a **non-secret marker** (`{"__inKeychain": true, "movedAt": <iso date>}`, ADR-43). In steady state there are zero secrets on files, with no migration command.
- **The marker is the answer to the split-brain** seen live on 2026-08-05: whether an install HAS a keychain depends on an optional native module, so a keychain-capable install can promote a credential that a file-only install on the same machine then cannot see. With the marker, the file-only side reports that the credential lives in the OS keychain this install cannot read (fix: install `@napi-rs/keyring`, or log in again) instead of claiming there are no credentials and advising a pointless re-login. A keychain-mode overwrite of a stale file copy buries it under the same marker; a tombstone delete removes the marker too. Every existing reader treats the marker as absence (it is an object where a key is a string, and it has no `accessToken`), so only the miss-path error messages consult it.
