...
| OpenRouter (translate, the universal fallback) | `moonshotai/kimi-k3` | `deepseek/deepseek-v4-pro` | `deepseek/deepseek-v4-flash` | (none) | Slugs re-verified 2026-08-12 against `GET /api/v1/models` (live), all with `tools` plus `tool_choice` (first verified 2026-07-18). GLM: `z-ai/glm-5.2`, `z-ai/glm-5-turbo` (vendor `z-ai`, not `zhipu`) |
...
- **`ProviderDef.catalogApi?: { url, auth?, stripPrefix? }`**. First entry: OpenRouter, `GET https://openrouter.ai/api/v1/models` (public, verified live 2026-08-13); a public catalogue sends no credential. `auth: true` (ADR-53) marks a list that needs the account key: `discover-catalog` then requires a `profile` in the body and resolves THAT profile's stored key, sent with the provider's own scheme to the provider's own documented endpoint and nowhere else. Entries: DeepSeek `GET https://api.deepseek.com/models` and Gemini's OpenAI-compat `GET .../openai/models` (docs-verified 2026-08-13; Gemini ids arrive decorated as `models/*`, cut via `stripPrefix`), OpenAI `GET /v1/models` (reference page unreachable to automated fetch on 2026-08-13; oldest stable shape, degrades soft). Moonshot documents no list endpoint (checked 2026-08-13) and stays out. Bare-id lists carry no window or pricing: the rows still buy autocomplete and existence. A profile with no resolvable key answers 502 and the input degrades to plain text.
- **`src/providers/catalog.ts`** fetches and normalizes to `{ id, name?, contextWindow?, supportsTools?, promptPrice?, completionPrice? }` (OpenRouter mapping: `top_provider.context_length` first, `context_length` as the fallback, because for a proxy the routed limit is what counts, §4quinquies point 4, and the recorded fixture shows a real 3.8x gap; `supported_parameters` containing `"tools"`; `pricing.prompt/completion` as USD-per-token strings). In-memory cache per provider, TTL 10 minutes; failures are never cached, and a 200 whose body is not the published shape counts as a failure, or it would be cached as an authoritative empty catalogue.
...
- **The mechanism is verified, the efficacy is not** (measured 2026-08-07, ADR-46). Fixtures pin when it fires, where it goes and that it is off by default (`test/edit-retry-hint.test.ts`), and a live probe through the real ingress fired it on 10/10 trials against 0/10 without the quirk. What is NOT established is that it helps: the M5 A/B needs a model that fails check 3 on its own, and the only model available on the reference machine scores 10/10 bare. On that model the flail this quirk targets never happened (0/10 in both arms) and the hint looked directionally worse, not significantly so at N=10 (ROADMAP M5 has the numbers). Off by default is therefore not a formality: nobody has shown this earns its tokens.
